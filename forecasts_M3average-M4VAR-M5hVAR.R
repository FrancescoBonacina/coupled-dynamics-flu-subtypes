...
# - hVAR
#
# The code for the VAR and hVAR predictions is based on: 
...
main_directory <- gsub('/code_for_github', '', wd)
outdir <- 'out'
...
composition_R2S <- function(v, map){
  ...
  if (map=='ilr'){
    ...
    print ("ERROR: map not known. Choose a map between 'ilr' and 'alr'")
  }
...
## ============== set the scenario ============= ###
scenario_base <- list(scenario_name='BASE',
                      rmv_non_sent=FALSE, 
                      sweek=17,
                      th_annual_cases=50,
                      coda_map='ilr',
                      infile='./input_forecast_subtype_compositions_2010-19.csv',
...
## ============== input data ================== ###
th_annual_cases <- scenario[['th_annual_cases']]
coda_map <- scenario[['coda_map']]
sweek <- scenario[['sweek']]
data <- read.csv(scenario[['infile']])
...
start_year <- 2010
years_to_predict <- scenario[['years_to_predict']]
groups <- c()
for (end_year in years_to_predict){
  my_group <- sprintf('group_2010%d', end_year-2001)
  groups <- c(groups, unique(data[[my_group]]))
}
groups <- sort(unique(na.omit(groups)))
...
nsamples <- 4*n.sim
save_diagnostic_plots <- scenario[['save_diagnostic_plots']]
if (save_diagnostic_plots){
  dir.create(file.path(outdir_path, sprintf('MC_diagnostic_plots_th%dcases_sweek%d_%s-map', th_annual_cases,sweek,coda_map)), showWarnings = FALSE)
}
...
for (gr in groups){
  ...
  for (end_year in years_to_predict){
    ...
    my_group <- sprintf('group_2010%d', end_year-2001)
    data1 <- data[data[,c(my_group)]==gr & !is.na(data[,c(my_group)]) & data$year>=start_year & data$year<=end_year,]
    ...
    s_obs <- lapply(Y, function(x){ ifelse(x[nrow(x),'B.']>=0.5, 'B', ifelse(x[nrow(x),'H1.']>=0.5, 'H1', ifelse(x[nrow(x),'H3.']>=0.5, 'H3', 'X'))) })   # the observed dominance state, among {B, H1, H3, X}
    ...
    s_hat_av <- sapply(y_av_simplex, function(y){ ifelse(y[1]>=0.5, 'B', ifelse(y[2]>=0.5, 'H1', ifelse(y[3]>=0.5, 'H3', 'X'))) })
    ...
    prob_s_is_H1_av <- sapply(samples_av_simplex, function(s){ sum(s[,2]>=0.5) / 1000 })                # ... same for H1, H3 and X
    ...
    mat_av[crows_av:(crows_av+N-1),6] <- t(matrix(unlist(s_obs), 1))              # the observed dominance state, among (B, H1, H3, X)
    ...
    mat_av[crows_av:(crows_av+N-1),17] <- s_hat_av                                                                            # the predicted dominance state, among (B, H1, H3, X)
    mat_av[crows_av:(crows_av+N-1),c(18,19,20,21)] <- cbind(prob_s_is_B_av, prob_s_is_H1_av, prob_s_is_H3_av, prob_s_is_X_av) # predicted probability for each dominance state (B, H1, H3, X)
    ...
    # try several lags for the autoregressive models:
    for (l in lag){
      #l <- 1
      ...
      s_hat_VAR <- sapply(y_VAR_simplex, function(y){ ifelse(y[1]>=0.5, 'B', ifelse(y[2]>=0.5, 'H1', ifelse(y[3]>=0.5, 'H3', 'X'))) })
      ...
      if (coef_corr_sig<99999){   # performance scores are defined only if the covariance matrix exists
        ...
        prob_s_is_H1_VAR <- sapply(samples_VAR_simplex, function(s){ sum(s[,2]>=0.5) / 1000 })                # ... same for H1, H3 and X
      ...
      if (coef_corr_sig<99999){ mat_VAR[crows_ar:(crows_ar+N-1),c(19,20,21,22)] <- cbind(prob_s_is_B_VAR, prob_s_is_H1_VAR, prob_s_is_H3_VAR, prob_s_is_X_VAR)} # predicted probability for each dominance state (B, H1, H3, X)
      ...
      ### if there are too few trajectories in the group, skip this group
      if (N<3){
        # fill matrix of results
      ...
      s_hat_hVAR <- sapply(y_hVAR_simplex, function(y){ ifelse(y[1]>=0.5, 'B', ifelse(y[2]>=0.5, 'H1', ifelse(y[3]>=0.5, 'H3', 'X'))) })
      ...
      prob_s_is_H1_hVAR <- sapply(samples_hVAR_simplex, function(s){ sum(s[,2]>=0.5) / 1000 })                # ... same for H1, H3 and X
      ...
      mat_hVAR[crows_ar:(crows_ar+N-1),c(19,20,21,22)] <- cbind(prob_s_is_B_hVAR, prob_s_is_H1_hVAR, prob_s_is_H3_hVAR, prob_s_is_X_hVAR) # predicted probability for each dominance state (B, H1, H3, X)
...
name_dfaverage <- sprintf('%s/4b_M3AVERAGE_%s_%diter.csv', outdir_path, scenario[['scenario_name']], n.iter)
write.table(df_average, name_dfaverage, sep = ',', dec = '.', row.names = F, col.names = T)
# VAR
df_VAR <- data.frame(mat_VAR)
colnames(df_VAR) <- cols_VAR
name_dfVAR <- sprintf('%s/4b_M4VAR_%s_%diter.csv', outdir_path, scenario[['scenario_name']], n.iter)
write.table(df_VAR, name_dfVAR, sep = ',', dec = '.', row.names = F, col.names = T)
# hVAR
df_hVAR <- data.frame(mat_hVAR)
colnames(df_hVAR) <- cols_hVAR
name_dfhVAR <- sprintf('%s/4b_M5HVAR_%s_%diter.csv', outdir_path, scenario[['scenario_name']], n.iter)
